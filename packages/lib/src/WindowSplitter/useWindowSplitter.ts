...
function useWindowSplitter(options: any): WindowSplitterTypes {
  const [size, setSize] = React.useState(0)
  const ref = React.useRef<Boolean | any>(null)
  const closed = React.useRef(true)

  const unclosed = (e: any) => e.target.className === SPLITTER_CLASSNAME && (closed.current = false)
